...
# -*- coding: utf-8 -*-
"""
FSM estimation of closure parameters from sparse field measurements 
...
#sensor placement
psi, _, _  = POD(w, nloc) 
_, _, pp = qr(psi.T, mode='economic', pivoting=True)

perm_matrix = np.zeros([(nx+1)*(ny+1),(nx+1)*(ny+1)])
for i in range(nloc):
    perm_matrix[i,pp[i]] = 1
C = perm_matrix[:nloc]
loc = pp[:nloc]

iloc,jind = np.meshgrid(loc,ind, indexing='ij')    
wobs = wfom[iloc,jind] + np.random.normal(0,sig,wfom[iloc,jind].shape)
z = wobs - wm[loc].reshape(-1,1) #mean subtraction at measurement locations
phi_s = phiw[loc,:]   
...
np.savez('./results/vm-fsm-sparse.npz', apod=atrue, agp=agp, afsm=afsm,\
