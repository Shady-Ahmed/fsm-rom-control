...
t_wind = 1.0 #assimilation window
...
perm_matrix = np.zeros([nx+1,nx+1])
for i in range(nloc):
...
uobs = ufom[iloc,jind] + np.random.normal(0,sig,ufom[iloc,jind].shape)
...
np.savez('./results/sparse_obs.npz', apod=atrue, agp=agp, afsm=afsm,\
                                     upod=upod, ugp=ugp, ufsm=ufsm,\
                                     lpod=lpod, lgp=lgp, lfsm=lfsm,\
                                     ufom=ufom, uobs=uobs, z=z, loc=loc)
